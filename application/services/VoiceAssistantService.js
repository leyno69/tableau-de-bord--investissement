...
export class VoiceAssistantService {
  ...
  async handle({ sessionId, audio, context = {}, wakeWord = 'Dis LEYNOR' } = {}) {
    ...
    const rawTranscript = await this.speechToTextProvider.transcribe({ audio, language: 'fr-FR' });
    ...
    const audioResponse = await this.textToSpeechProvider.synthesize({ text: answer.answer, language: 'fr-FR' });
    const response = new VoiceResponse({
      text: answer.answer,
      audio: audioResponse,
      confidence: Number.isFinite(confidence) ? Math.min(1, Math.max(0, confidence)) : 0,
      risks,
      metadata: { model: answer.model ?? null }
    });
