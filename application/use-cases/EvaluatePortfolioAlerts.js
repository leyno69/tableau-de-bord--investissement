...
export class EvaluatePortfolioAlerts {
  constructor({ rules, clock = () => new Date(), idGenerator = randomUUID } = {}) {
  ...
  async execute({ portfolioId, context, existingFingerprints = [] }) {
    ...
    const events = [];
    const evaluatedAt = EvaluatePortfolioAlerts.#requireDate(this.clock());
    ...
    for (const rule of this.rules) {
      ...
      for (const candidate of normalizedCandidates) {
        ...
        const fingerprint = EvaluatePortfolioAlerts.#requireText(candidate.fingerprint, 'fingerprint');
        if (known.has(fingerprint)) {
          continue;
        }

        events.push(new AlertEvent({
