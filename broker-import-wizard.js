...
function buildWizard(shell) {
  if (!shell || shell.dataset.importWizardMounted === 'true') return shell;
  ...
  header.innerHTML = `
    <div><p class="import-wizard__eyebrow">IMPORT DE PORTEFEUILLE</p><h2 id="importWizardTitle">${stepTitle('broker')}</h2></div>
  ...
  [...shell.children].filter(node => node !== closeButton).forEach(node => body.append(node));
  ...
  [...body.children].forEach(node => {
    ...
    if (!node.closest?.('[data-wizard-panel]') && !node.matches?.('script, style')) {
  ...
  let current = 0;
  const selectedFile = () => shell.querySelector('#brokerImportFile')?.files?.[0] || null;
  const canAdvance = index => {
  ...
  function show(index, { focus = true } = {}) {
    ...
    if (shell.dataset.wizardStep !== String(current)) shell.dataset.wizardStep = String(current);
    const title = shell.querySelector('#importWizardTitle');
    const nextTitle = stepTitle(STEP_DEFINITIONS[current].key);
    ...
    panels.forEach((panel, panelIndex) => {
      ...
      if (panel.getAttribute('aria-hidden') !== String(!active)) panel.setAttribute('aria-hidden', String(!active));
    });
    ...
    shell.querySelectorAll('[data-wizard-jump]').forEach((button, buttonIndex) => {
      ...
      const ariaCurrent = buttonIndex === current ? 'step' : 'false';
  ...
  shell.querySelector('[data-wizard-next]')?.addEventListener('click', () => {
    ...
    if (current >= STEP_DEFINITIONS.length - 1) return closeButton?.click();
    show(current + 1);
  });
  ...
  shell.addEventListener('change', event => {
    if (event.target?.id === 'importBroker' || event.target?.id === 'brokerImportFile') updateControls();
  });
  shell.addEventListener('input', updateControls);

  const status = shell.querySelector('#brokerImportStatus');
  if (status) {
    new MutationObserver(updateControls).observe(status, { childList: true, subtree: true, characterData: true });
  }
...
function ensureAssets() {
  if (document.querySelector('link[data-import-wizard]')) return;
...
function mount() {
  ...
  return buildWizard(shell);
}

function startDiscovery() {
  if (mount() || discoveryObserver) return;
  discoveryObserver = new MutationObserver(() => mount());
...
export { STEP_DEFINITIONS, buildWizard, mount, startDiscovery };
