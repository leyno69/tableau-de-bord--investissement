...
export function loadServerConfig(environment = process.env) {
  if (!environment || typeof environment !== 'object') throw new TypeError('environment doit être un objet.');
  ...
  return Object.freeze({
    ...
    storage: Object.freeze({
      ...
      exchangeRateCachePath: text(environment.EXCHANGE_RATE_CACHE_PATH ?? DEFAULTS.exchangeRateCachePath, 'EXCHANGE_RATE_CACHE_PATH')
    }),
    instrumentCatalog: Object.freeze({
      repository: instrumentRepository,
      filePath: text(environment.INSTRUMENT_CATALOG_PATH ?? DEFAULTS.instrumentCatalogPath, 'INSTRUMENT_CATALOG_PATH')
    }),
...
function integer(value, field, { minimum, maximum = Number.MAX_SAFE_INTEGER }) {
  ...
  if (!Number.isInteger(number) || number < minimum || number > maximum) throw new RangeError(`${field} doit être un entier compris entre ${minimum} et ${maximum}.`);
  return number;
}
