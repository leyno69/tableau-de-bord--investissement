...
export class ConcentrationAlertRule {
  ...
  evaluate({ allocation }) {
    if (!allocation || !Array.isArray(allocation.holdings)) return [];
    ...
    return allocation.holdings
      ...
      .map(item => ({
        ...
        context: { assetId: item.assetId, weight: item.weight, threshold: this.threshold }
      }));
  }
}
...
export class DrawdownAlertRule {
  ...
  evaluate({ analytics }) {
    ...
    if (!drawdown || drawdown.rate === null || Math.abs(drawdown.rate) < this.threshold) return [];
...
export class StaleMarketDataAlertRule {
  constructor({ maxAgeMilliseconds, severity = 'WARNING', now = () => new Date() }) {
    ...
    if (typeof now !== 'function') throw new TypeError('now doit être une fonction.');
    this.severity = severity;
    this.now = now;
    Object.freeze(this);
  }
  ...
  evaluate({ quotes }) {
    ...
    const nowTimestamp = current instanceof Date ? current.getTime() : Date.parse(current);
    if (!Number.isFinite(nowTimestamp)) throw new TypeError('now doit retourner une date valide.');
