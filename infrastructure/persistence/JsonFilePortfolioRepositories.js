...
class JsonPortfolioStore {
  ...
  async load() {
    ...
    for (const field of ['transactions', 'snapshots', 'alerts', 'preferences']) {
      if (!Array.isArray(payload[field])) throw new TypeError(`${field} doit être un tableau.`);
    }
    this.state = {
  ...
  mutate(action) {
    ...
    return this.#schedulePersist().then(() => result);
  }
...
export class JsonTransactionRepository {
  ...
  async listByPortfolio(portfolioId) {
    const id = text(portfolioId, 'portfolioId');
    return Object.freeze([...this.store.state.transactions.values()].filter(item => item.portfolioId === id).sort((a, b) => Date.parse(a.executedAt) - Date.parse(b.executedAt) || a.id.localeCompare(b.id)));
  }
}
...
export class JsonSnapshotRepository {
  ...
  async save(snapshot) {
    ...
    return this.store.mutate(state => {
      ...
      if (existing && JSON.stringify(existing.toJSON()) !== JSON.stringify(snapshot.toJSON())) throw new RangeError('Un snapshot différent existe déjà pour cet horodatage.');
  ...
  async listByPortfolio(portfolioId, { from = null, to = null } = {}) {
    const id = text(portfolioId, 'portfolioId');
    const fromTime = from == null ? -Infinity : date(from, 'from');
    const toTime = to == null ? Infinity : date(to, 'to');
...
export class JsonAlertRepository {
  ...
  async saveAll(events) {
    ...
    return this.store.mutate(state => {
      for (const event of events) {
        ...
        const duplicate = [...state.alerts.values()].some(item => item.portfolioId === event.portfolioId && item.fingerprint === event.fingerprint);
        if (!duplicate) state.alerts.set(event.id, event);
      }
  ...
  async listByPortfolio(portfolioId) {
    const id = text(portfolioId, 'portfolioId');
    return Object.freeze([...this.store.state.alerts.values()].filter(item => item.portfolioId === id).sort((a, b) => Date.parse(a.triggeredAt) - Date.parse(b.triggeredAt) || a.id.localeCompare(b.id)));
  }
...
export class JsonPreferencesRepository {
  ...
  async save(preferences) {
    ...
    return this.store.mutate(state => { state.preferences.set(preferences.portfolioId, preferences); return preferences; });
  }
  async findByPortfolio(portfolioId) { return this.store.state.preferences.get(text(portfolioId, 'portfolioId')) ?? null; }
}

function hydrateSnapshot(value) {
  return new PortfolioSnapshot({ ...value, totalValue: new Money(value.totalValue.amount, value.totalValue.currency) });
}
function emptyState() { return { transactions: new Map(), snapshots: new Map(), alerts: new Map(), preferences: new Map() }; }
